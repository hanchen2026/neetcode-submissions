    ...
    public boolean isAlienSorted(String[] words, String order) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            for (int j = 0; j < shorter; j++) {
                ...
                //else if there is an issue, set a flag (you could break too but im lazy)
                if (alphabet.get(comparatorB) < alphabet.get(comparatorA)) issue = true;
            }
            
            //if there is an issue, or an exact match with the following word being longer, return false. 
