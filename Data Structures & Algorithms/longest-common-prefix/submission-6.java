    ...
    public String longestCommonPrefix(String[] strs) {
        ...
        while (idx < strs[0].length()) {
            ...
            for (int i = 1; i < strs.length; i++) {
                if (strs[i].length() <= 0) return "";      //if empty string is enocountered, return "".
                else if (idx >= strs[i].length()) return strs[0].substring(0, idx);    //if the first string is longer than some later string, return current prefix
